...
def extract_au(base_dir, img_dir):
    """
    [Optimization] 
    Python-native implementation of Blink Detection (AU45).
    Replaces the heavy dependency on OpenFace C++ binary.
    Calculates Eye Aspect Ratio (EAR) from landmarks.
    """
    print(f"[INFO] ===== Using Python-native EAR algorithm to generate au.csv (Skipping OpenFace) =====")
    
    csv_path = os.path.join(base_dir, 'au.csv')
    if os.path.exists(csv_path):
        print(f"[INFO] au.csv already exists, skipping calculation.")
        return

    # 1. 加载人脸关键点检测器 (复用环境中已有的 face_alignment 库)
    import face_alignment
    import torch
    ...
    au_data = []
    print(f"[INFO] Processing {len(image_paths)} frames for blink detection...")
    
    # 4. 逐帧处理
    for idx, img_path in enumerate(tqdm.tqdm(image_paths, desc="Extracting AU45")):
        try:
            # 读取图片并预测关键点
            ...
            if preds is not None and len(preds) > 0:
                ...
                # --- EAR 到 AU45 的映射逻辑 ---
                # 经验阈值：通常 EAR < 0.2 视为闭眼
                # OpenFace 的 AU45 范围通常是 0 (睁眼) 到 5 (闭眼)
                if avgEAR < 0.18:
                    au45 = 5.0  # 完全闭眼
                elif avgEAR > 0.30:
                    au45 = 0.0  # 完全睁眼
                    ...
                    # 中间线性插值
                    au45 = (0.30 - avgEAR) / (0.30 - 0.18) * 5.0
            
        except Exception as e:
            print(f"[WARN] Frame {idx} detection failed: {e}")
            au45 = 0.0

        # 5. 构造符合 OpenFace 格式的数据行
        # 必须包含 frame, timestamp, confidence, success, AU45_r
        row = {
            'frame': idx + 1,
            'timestamp': idx * 0.04,  # 假设 25fps，每帧 0.04 秒
            'confidence': 0.98,
            'success': 1,
            ' AU45_r': float(au45)  # 注意：列名前有空格，保持与 OpenFace 格式一致
        }
        au_data.append(row)
    ...
    print(f"[SUCCESS] Python-generated AU data saved to {csv_path}")
    print(f"[INFO] ===== AU extraction completed =====")
...
if __name__ == '__main__':
    ...
    # extract AU features (blink detection) - Python-native implementation
    if opt.task == -1 or opt.task == 8:
        extract_au(base_dir, ori_imgs_dir)
